...
ADDR3L = (IP_SV2, PORT_SV3L)
ADDR3W= (IP_SV2, PORT_SV3W)
...
def Worker(path,address):
    ...
    while True:
        ...
        if result[2]:
            conn.send(str(result[2]).encode(FORMAT))
            print("Receiving :", result[2])
            for file_name in result[2]:
                recive_file(conn, file_name)
...
def Main(): 
    ...
    WorkerT = ThreadWithReturnValue(target=Worker, args=(os.getcwd(),ADDR2W,))
    ListenerT = ThreadWithReturnValue(target=Listener,args=(ADDR2L,))
    WorkerT2 = ThreadWithReturnValue(target=Worker, args=(os.getcwd(),ADDR3W,))
    ListenerT2 = ThreadWithReturnValue(target=Listener,args=(ADDR3L,))
